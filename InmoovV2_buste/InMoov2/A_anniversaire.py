...
def anniversaire():
    ...
    if os.path.exists('data/bdamis.csv'):
        ...
        annee = int(time.strftime('%Y')) # pour calcul de l age
        # mise en forme de la date pour rappel a j-7 et j-1
        jmoins1= str(int(time.strftime('%d'))-1)+"-"+time.strftime('%m')
        jmoins7 =str(int(time.strftime('%d'))-7)+"-"+time.strftime('%m')
        flag=0
        # creation du dictionnaire des saints index sur la date calendrier cregorien
        saint = {
	    ...
	    # Boucle lecture lignes une par une colonne 4 (champ anniversaire).
        for ligne in reader:
            #print(ligne[4])
            ...
            if (jmoins1 == ligne[4]):
                age = annee - int(ligne[3])
                flag=1
                i01.speakBlocking(u"demain c'est l'anniversaire de "+ ligne[0] + str(age)  +" ans")
            if (jmoins7 == ligne[4]):
                age = annee - int(ligne[3])
                flag=1
                i01.speakBlocking(u"c'est l'anniversaire de "+ ligne[0] + str(age)  +" ans dans 7 jour")
        f.close()
         # si pas anniversaire on faite les saints 
        if flag==0 :
            i01.speakBlocking(u"c'est l'anniversaire de personne . nous faitons " + (saint.get(annivsaint)) )
            
    else :   # le fichier des amis n existe pas
        i01.speakBlocking(u"tu dois d'abord créer ton annuaire d'amis . Dis ANNUAIRE pour le créer")

def chercheamis(nomamis):
    # if fichier des amis existe 
    if os.path.exists('data/bdamis.csv'):
        resultats = []
        flag = 0
        with open('data/bdamis.csv', 'rb') as csvfile:
            reader = csv.reader(csvfile,delimiter=";")
            for row in reader:
                #print(row[0])
                if row[0] == nomamis :
                    resultats.append(row[0])
                    resultats.append(row[1])
                    resultats.append(row[2])
                    resultats.append(row[3])
                    resultats.append(row[7])
                    i01.speakBlocking(resultats[0]+ u" fait parti de tes amis, est né le : "+resultats[1]+"/"+resultats[2]+"/"+resultats[3]+u" sa dernière visite remonte au : "+resultats[4])
                    flag=1
            if flag == 0 :
                i01.speakBlocking(u"Désolé je ne connais pas ,"+ nomamis)
        csvfile.close()
    else :   # le fichier des amis n existe pas
        i01.speakBlocking(u"tu dois d'abord créer ton annuaire d'amis . Dis annuaire pour le créer")
